...
from lam.modules.flow_decoder import FlowDecoder
...
class LatentActionModelV9(nn.Module):
    ...
    variant:
      ...
      'D': V8 encoder + flow decoder (z_actor -> flow -> warp crop_t)
    """
    ...
    def __init__(
        ...
        if variant == "C":
            ...
            z_dec_bg = z_bg_dim if use_bg_slot else 0
            if variant == "D":
                self.use_flow_decoder = True
                self.recon_decoder = FlowDecoder(
                    z_actor_dim=z_dim,
                    z_bg_dim=z_dec_bg,
                    crop_size=crop_size,
                )
            else:
                self.use_flow_decoder = False
                self.recon_decoder = ReconDecoder(
                    z_actor_dim=z_dim,
                    z_bg_dim=z_dec_bg,
                    crop_size=crop_size,
