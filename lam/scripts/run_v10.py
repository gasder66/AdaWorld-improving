"""
V10: V6c + Shared VAE 训练脚本.

V6c 有重建无聚类 → V10 用 shared VAE 修复聚类, 保留重建。

Loss = L_recon + β·L_KL + λ·L_obj_recon + μ·L_delta + ν·L_contrast

用法:
  CUDA_VISIBLE_DEVICES=2 PYTHONPATH=lam python lam/scripts/run_v10.py \\
      --name v10_stage1 --batch_size 16 --steps 5000 --lr 2.5e-4
"""
...
def main():
    ...
    print(f"  recon_w={args.recon_weight}, kl_beta={args.kl_beta}, "
          f"obj_recon_w={args.obj_recon_weight}")
    print(f"  delta_w={args.delta_weight}, contrast_w={args.contrast_weight}")
    ...
    with torch.no_grad():
        for batch in eval_loader:
            ...
            v_mask = batch.get("valid_mask", None)
            if v_mask is None:
                # DiskSyntheticDataset uses actions >= 0 as valid
                v_np = act_np >= 0
            else:
    ...
    psnr_full = -10 * np.log10(float(recon_mse_arr.mean()) + 1e-10)
    ...
    print(f"  Overall NMI: {nmi:.4f}  (V6c: 0.0525, V8: 0.7723)")
    print(f"  ARI: {ari:.4f}")
    print(f"  Per-Slot NMI avg: {np.mean(nmi_per):.4f}  (V6c: 0.3885)")
    print(f"  Actor Leakage: {leak:.4f}  (V6c: 1.0, chance: {1/len(np.unique(slots_arr)):.4f})")
    print(f"  Action Probe: {act_acc:.4f}  (chance: {1/n_clusters:.4f})")
    print(f"  Full-frame PSNR: {psnr_full:.2f} dB  (V6c: 27.35)")
    ...
    results = {
        ...
        "full_frame_psnr": round(float(psnr_full), 2),
    ...
    print(f"\n  模型保存: {RESULTS_DIR}/model_{args.name}.pt")
    print(f"{'='*60}\n")
