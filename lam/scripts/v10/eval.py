...
用法:
  PYTHONPATH=lam python lam/scripts/eval_v10.py --dataset synthetic --config stage1 --gpu 0
"""
...
def main():
    ...
    # Load latents
    latents_path = os.path.join(RESULTS_DIR, "latents.npz")
    ...
    print(f"\n{'='*60}")
    print(f"V10 Evaluation: {VERSION}/{args.dataset}/{args.config}")
    ...
    results = {
        "model": f"V10 ({args.dataset}/{args.config})",
    ...
    try:
        ...
        plt.tight_layout()
        umap_path = os.path.join(RESULTS_DIR, "umap.png")
    ...
    model.load_state_dict(torch.load(
        os.path.join(RESULTS_DIR, "model.pt"), map_location=device
    ))
    model.eval()
