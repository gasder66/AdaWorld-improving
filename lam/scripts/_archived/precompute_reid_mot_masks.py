"""预生成 COCO YOLO + BoT-SORT(ReID) 的 MOT masks。"""
...
cfg = {
    ...
    "proximity_thresh": 0.5, "appearance_thresh": 0.8, "with_reid": True, "model": "auto",
}
cfg_path = "/tmp/botsort_reid.yaml"
...
for split in ["train", "test"]:
    ...
    out_dir = f"/home/xiaojy/projects/AdaWorld-improving/result/v6_a2d/mot_masks_reid/{split}"
    os.makedirs(out_dir, exist_ok=True)

    for idx in range(len(dataset)):
        sample = dataset[idx]; video_np = sample["videos"].numpy()
        T = video_np.shape[0]; pred_masks = torch.zeros(T, 4, 256, 256)
