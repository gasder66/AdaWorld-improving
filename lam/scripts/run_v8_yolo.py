"""
V8 Stage 4: Train V8 on YOLO-detected A2D data (large scale).

与 Stage 2B 的区别:
  - YOLO 检测任意帧 (不受标注帧限制),样本量 ~28000 vs 248
  - COCO→A2D actor type 映射
  - GT action labels 仅在标注帧 (大部分 action=-1,训练用 motion_loss)
  - disk cache 加速 YOLO 推理

用法:
  CUDA_VISIBLE_DEVICES=2 PYTHONPATH=lam python lam/scripts/run_v8_yolo.py \\
      --name v8_yolo --batch_size 16 --steps 5000 --max_samples 2000
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"V8 Stage 4: YOLO-detected A2D (large scale)")
    ...
    # === Eval: collect latents (only samples with GT action labels) ===
    model.eval()
    ...
    with torch.no_grad():
        for batch in eval_loader:
            ...
            for b in range(B):
                for t in range(T1):
                    for k in range(K):
                        if v_np[b, t, k] and act_np[b, t, k] >= 0:  # only labeled
    ...
    results = {
        "architecture": "v8_mot_lam", "stage": "stage4_yolo",
